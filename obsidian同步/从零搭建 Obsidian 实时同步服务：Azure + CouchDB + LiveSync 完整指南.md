...
**现象**：
```
电脑端图片显示正常，手机/iPad 端文件列表中能看到图片（已同步到本地），
但笔记在阅读模式（Reading view）下无法渲染图片，编辑模式正常。
iPad 端 LiveSync 弹出提示：
Something went wrong on reading xxxx.png from database
```

**排查误区**：

⚠️ 该报错看似是同步链路故障，实际是红鲱鱼——图片文件其实早已完整落盘。

判断数据是否真的同步到位：
1. 在设备本地（Obsidian 文件列表或文件管理器）确认图片文件存在且大小正常
2. 切换到编辑模式查看同一笔记，若能显示 ⇒ 数据已同步，问题出在渲染层而非同步层
3. 开启飞行模式再打开图片，离线也能打开 ⇒ 内容已在本地，与服务器完全无关

**原因**：
Obsidian 移动端阅读视图的一类已知渲染缺陷：
- 笔记中使用 HTML 标签 `<img src="本地路径">` 而非标准嵌入语法 `![[xxx.png]]` 时，
  移动端 Reading view 不解析本地路径（Obsidian 论坛有同款 Bug 报告）
- 路径含中文、空格等字符时，阅读视图的 URL 编码处理与实际存储路径不一致

失败的资源请求会触发 LiveSync 的读取钩子，从而抛出 "reading from database" 错误，把排查方向带偏。

**解决**：
- 将 HTML img 标签改为标准嵌入语法 `![[xxx.png]]`
- 升级 Obsidian 至新版本观察是否修复
- 💡 排查口诀：先确认「文件是否真的到了本地」，再怀疑「同步链路」；插件报错不一定是根因
