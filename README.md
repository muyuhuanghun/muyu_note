...
- **标题格式**：`# YYYY-MM-DD 星期X`
- **模块结构**：今日计划 → 学习笔记 → 今日完成 → 总结与反思 → 每日自动总结
- **文档引用**：在正文中直接标识修改了哪些文档，不使用 `[[ ]]` 双向链接
- **自动总结**：由 git post-commit hook 自动生成，包含当日提交记录和变更概览
...
15. 每日学习日记（格式规范：纯文本标题 + 正文标识修改文档，不使用双向链接）
16. Obsidian 多设备同步方案（Git + GitHub 方案分析、自建 CouchDB + LiveSync 实时同步方案）
