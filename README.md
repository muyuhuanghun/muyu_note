...
- `CANN/`
  ...
  - 附官方资料库快照 `CANN-assets-20260813/`（技术博客、算子开发教程、快速上手文档）与学习路线笔记
- `PBLF/`
  - 算法练习题与练习记录
- `leetcode/`
  - LeetCode 刷题笔记（每日一题 + 按专题分类：哈希、双指针、滑动窗口、子串等）
- `数字信息处理/`
  - 数字信息处理课程笔记（MSP430 / CCS 环境搭建、踩坑总结、UART 实验与 IDE 排障记录，附板卡说明 PDF）
  - `异构多核音频处理/` — 异构多核音频处理课程详解（MSP430 MCU → C674x DSP → DM8168 异构多核），含 `VSCode嵌入式开发环境/` VSCode 替代 Keil 搭建 STM32 开发环境指南
  - 综合设计 I 完成计划（2026-09）
...
- `量化交易入门(X)/`
  - 量化交易入门笔记
  - Hull《期权、期货及其他衍生品》38 章逐章笔记
  - `入门课程体系/`：全网权威教材与课程调研后整理的量化交易入门课程（12 篇代码驱动笔记：全景图、数据获取、收益率度量、回测陷阱、动量/均值回归策略、Fama-French 多因子、组合优化与风险控制、交易成本、金融机器学习与书单索引）
- `灵感/`
  - 随手记录的想法与灵感片段
- `obsidian同步/`
...
8. 量化交易入门（Hull《期权、期货及其他衍生品》38 章笔记 + 12 篇入门课程体系：策略、回测、多因子、风控与金融 ML）
9. GitHub 与开发环境使用、Linux 基础命令与服务器管理
10. CET-6 / 英语备考复习（真题、句式分类、话题语料、范文）
11. Obsidian 多设备同步方案（自建 Azure + CouchDB + LiveSync 实时同步，含完整排错记录）
12. 数字信息处理课程笔记（MSP430 板卡调试、CCS 环境搭建与踩坑总结，含异构多核音频处理与 VSCode 嵌入式开发环境）
13. LeetCode 刷题（每日一题 + 哈希、双指针、滑动窗口、子串等专题）
