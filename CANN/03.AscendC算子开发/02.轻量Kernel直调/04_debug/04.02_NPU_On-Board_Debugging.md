...
source_path: tutorials/ascendc_operator_development_light/04_debug/04.02_NPU_On-Board_Debugging.ipynb
...
> 📚 上游 Notebook：[tutorials/ascendc_operator_development_light/04_debug/04.02_NPU_On-Board_Debugging.ipynb](https://gitcode.com/cann/cann-learning-hub/blob/master/tutorials/ascendc_operator_development_light/04_debug/04.02_NPU_On-Board_Debugging.ipynb)
...
算子调试是算子开发验证阶段的核心环节，通过精准打印运行时数据与日志信息，可快速定位算子在NPU硬件执行过程中的逻辑错误、数据异常等问题。本节聚焦NPU域上板打印能力，介绍DumpTensor和printf两类接口，其中DumpTensor用于打印指定Tensor的数据，printf主要用于打印标量和字符串信息。本文将从接口定位、使用规范、实操要点三个维度，系统介绍算子调试的核心方法。
...
两类接口均需**在算子kernel侧实现代码中**调用，且需确保代码已完成NPU硬件适配（如包含必要的头文件、遵循算子开发语法规范）。调用位置无严格限制，可根据调试需求，在算子初始化、核心计算逻辑、结果输出等关键节点插入接口调用语句。

#### 2.2 DumpTensor接口功能说明与使用示例

DumpTensor用于在算子执行过程中打印指定Tensor的内容，辅助调试与分析。接口支持附加自定义信息（desc参数，仅支持uint32_t类型），例如行号或标识符，以便在多处调用时区分不同输出。
...
<table border="1" cellpadding="8" cellspacing="0" style="margin-left: 0; text-align: left;">
  ...
  <tbody>
    <tr>
      ...
      <td>待打印的Tensor，支持LocalTensor（位于Unified Buffer/L1/L0C）或GlobalTensor（位于Global Memory）。</td>
    </tr>
    <tr>
...
printf接口提供格式化输出能力，其语法与标准C/C++ printf兼容，便于在算子核函数中打印变量值、调试信息等。
...
fmt参数是格式控制字符串，由普通字符和转换说明组成。普通字符原样输出，转换说明以 % 开头，用于指定后续参数的类型和输出格式。在NPU域调试时，转换说明所支持的数据类型如下表所示：
...
在算子核函数中，可根据需要在关键位置插入printf语句，例如打印变量值、循环索引或调试标记。示例如下：
...
本节将直接基于[Add算子](https://gitcode.com/cann/cann-learning-hub/blob/master/tutorials/ascendc_operator_development_light/04_debug/src/04.02/add_custom.asc)，演示如何通过添加 `DumpTensor` 和 `printf` 接口，在算子核函数的关键位置输出调试信息，帮助开发者直观了解NPU上的数据流动与计算状态。
...
插入`DumpTensor`后的KernelAdd::Compute函数如下：
```
...
插入`printf`后的KernelAdd::Init函数如下：
...
将如上添加打印信息的改动写入Sources/04.02/add_custom.asc文件中：

```bash
%%writefile Sources/04.02/add_custom.asc
...
```bash
%%writefile Sources/04.02/CMakeLists.txt
...
实践要求：请修改如下add_custom.asc源码文件，在核函数的Compute函数中调用DumpTensor接口添加打印语句。具体要求如下：

- 仅在逻辑ID为2的AI Core最后一次执行Compute函数时触发打印；

- 打印zLocal张量的前8个元素；

- 使用带Tensor shape的DumpTensor接口，并按shape为 (2, 8) 的格式显示。
...
```
[AIV Block 2] DumpTensor: desc=0, addr=0xa00, data_type=float32, position=UB, dump_size=8
